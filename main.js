...
const BLOCK_TYPES = {
  1: { name: 'Grass', color: 0x55aa55 },
  2: { name: 'Dirt', color: 0x9b7653 },
  3: { name: 'Stone', color: 0x777777 },
  4: { name: 'Wood', color: 0x8b5a2b },
  5: { name: 'Leaves', color: 0x22aa22 }
};
...
function makeBlock(type) {
  const mat = new THREE.MeshStandardMaterial({ color: BLOCK_TYPES[type].color });
...
function isSolidBlock(x, y, z) {
  ...
  if (!b) return false;
  // Leaves are not solid (can walk through)
  return b.userData.blockType !== 5;
}
